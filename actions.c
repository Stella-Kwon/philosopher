...
/*   Updated: 2024/07/05 02:25:28 by sukwon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	lock_both_fork(t_threads *philo)
{
	...
	{
		if (unlock_mutex(&philo->data->forks[left_index], "philo->data->forks[right_index]") == EXIT_FAILURE)
			return (EXIT_FAILURE);
		return (EXIT_FAILURE);
	}
...
int		eating(t_threads *philo)
{
	...
	if (lock_mutex(&(philo->lastmeal_lock), "data->lastmeal_lock") == EXIT_FAILURE)
        return EXIT_FAILURE;
	philo->last_meal = get_time(philo->data);
	if (philo->last_meal == -100)
	{
		if (unlock_both_fork(philo) == EXIT_FAILURE)
		{
            if (unlock_mutex(&(philo->lastmeal_lock), "data->lastmeal_lock") == EXIT_FAILURE)
                return EXIT_FAILURE;
